...
def _coalesce_text(pieces) -> str:
    """
    Accepts list/tuple/any and returns a single clean string.
    - If elements are dicts, tries keys: body_en, body, text, content.
    - Falls back to str(...) for everything else.
    """
    if not pieces:
        return ""
    out = []
    keys = ("body_en", "body", "text", "content")
...
def topic_tts_path(topic: str, lang: str = "en", out_dir: str = "out") -> str:
    """Return the path for today's TTS file, e.g., out/tts_tech_en_2025-09-02.mp3"""
    os.makedirs(out_dir, exist_ok=True)
    date_str = dt.datetime.utcnow().strftime("%Y-%m-%d")
    fname = f"tts_{_slug(topic)}_{lang}_{date_str}.mp3"
    return str(Path(out_dir) / fname)


def get_or_generate_topic_tts(
    text: str,
    topic: str,
    lang: str = "en",
    voice: str = "alloy",
    ...
    """
    Generate TTS for the given text if today's mp3 doesn't exist; otherwise reuse it.
    ...
    path = topic_tts_path(topic=topic, lang=lang, out_dir=out_dir)
    ...
    client = get_openai_client()  # your existing helper that reads .env

    # Stream/save to file (OpenAI Python SDK 1.x)
    # Note: httpx pinned to 0.27.x in requirements to avoid proxies arg issues.
    from openai import OpenAI

    # Safety: ensure parent dir exists
    Path(path).parent.mkdir(parents=True, exist_ok=True)

    # Streaming to file to avoid big memory blobs
    with client.audio.speech.with_streaming_response.create(
        model=model,
        voice=voice,
        input=text[:6000],  # mild cap
    ) as resp:
...
class OpenAISummarizer:
    ...
    def summarize(self, pieces) -> str:
        blob = _coalesce_text(pieces)
        if not blob:
            return ""
        # short inputs: don’t over-summarize
        if len(blob.split()) < 18:
            return blob

        # you can keep your existing prompt; this is just the safe input path
        resp = self.client.chat.completions.create(
            model=self.model,
            messages=[
                {
                    "role": "system",
                    "content": "Summarize the following news content concisely and factually.",
                },
...
def summarize_selected(
    stories: list[dict],
    oa_model: str,
    articles_map: dict | None = None,
) -> None:
    sm = OpenAISummarizer(model=oa_model)

    def _body_from_articles_map(url: str) -> str:
        if not articles_map:
            return ""
        rec = articles_map.get(url, "")
        # rec might be string (old cache) or dict (new cache)
        if isinstance(rec, dict):
            return rec.get("body_en") or rec.get("body") or ""
        return rec or ""
    ...
    for s in stories:
        body = ""
        # prefer canonical, then fall back to other links in the cluster
        urls = [s.get("canonical_url", "")] + list(s.get("sources", []))
        for u in urls:
            if not u:
                continue
            body = _body_from_articles_map(u)
            if body and len(body) >= 200:  # cheap sanity
                break

        parts = [s.get("title_en") or s.get("title") or "", body]
